...
JNIEXPORT jstring JNICALL Java_com_litongjava_media_NativeMedia_toMp3(JNIEnv *env, jclass clazz, jstring inputPath) {
  ...
  // Create output filename: if input filename has an extension, replace it with .mp3, otherwise append .mp3
  char *output_file = NULL;
  size_t input_len = strlen(input_file);
  const char *dot = strrchr(input_file, '.');
  if (dot != NULL) {
    // Calculate base length without extension
    size_t base_len = dot - input_file;
    output_file = (char *) malloc(base_len + 4 + 1); // base + ".mp3" (4 characters) + null terminator
    if (!output_file) {
    ...
    // If no extension found, simply append .mp3
  ...
  // Create output format context for MP3
  ...
  // Find the MP3 encoder using libmp3lame
  ...
  encoder_context->sample_fmt = AV_SAMPLE_FMT_S16P; // libmp3lame typically uses s16p
  // For some encoders, AV_SAMPLE_FMT_S16 might be acceptable as well
  ...
  // Enable global header if needed by container format
  ...
  // Open output file for writing if required
  ...
  while (av_read_frame(input_format_context, input_packet) >= 0) {
    ...
    if (input_packet->stream_index == audio_stream_index) {
      ...
      while (1) {
        ...
        // Ensure output frame is writable
        ...
        // Convert audio samples using the resampler
  ...
  // On success, set the result to the output file path
